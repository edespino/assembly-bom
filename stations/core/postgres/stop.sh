#!/usr/bin/env bash
# --------------------------------------------------------------------
# File     : stations/core/postgres/stop.sh
# Purpose  : Stop PostgreSQL server for postgres15, postgres16, etc.
# Inputs   :
...
if ! pg_ctl status -D "$PGDATA_DIR" &> /dev/null; then
  ...
  log "Data directory: $PGDATA_DIR"
  section_complete "stop" "$start_time"
  exit 0
fi
...
log "Command: pg_ctl -D $PGDATA_DIR -s stop"
echo ""

pg_ctl -D "$PGDATA_DIR" -s stop

echo ""
log "PostgreSQL server stopped successfully"
log "Data directory: $PGDATA_DIR"
log ""
log "To start the server again:"
log "  ./assemble.sh --run --component $NAME --steps start"
log "  OR"
log "  source $ENV_FILE"
log "  pg_ctl -D $PGDATA_DIR -l $PGDATA_DIR/logfile -s start"
