#!/usr/bin/env bash
# --------------------------------------------------------------------
# File     : stations/build-install-pg_filedump.sh
...
if [[ -f "$CLOUDBERRY_PATH_SH" ]]; then
  ...
  echo "[build-install-pg_filedump] ERROR: greenplum_path.sh not found at $CLOUDBERRY_PATH_SH"
  exit 1
fi
...
if [[ -f "$CLOUDBERRY_DEMO_ENV" ]]; then
  ...
  echo "[build-install-pg_filedump] ERROR: gpdemo-env.sh not found at $CLOUDBERRY_DEMO_ENV"
  exit 1
fi
...
section_complete "build-install-pg_filedump" "$start_time"
